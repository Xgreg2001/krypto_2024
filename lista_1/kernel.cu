...
__device__ uint32_t NO_FF(uint32_t a, uint32_t b, uint32_t c, uint32_t d,
                          uint32_t t, uint32_t s, uint32_t orig) {
  return _rotr((a - b), s) - F(b, c, d) - orig - t;
}

__global__ void validateCandidates(const uint32_t *state, uint32_t *candidates,
                                   uint8_t *found, size_t batch_size) {
  ...
  words[0] = NO_FF(a, b, c, d, 0xD76AA478, 7, orig);
  ...
  words[1] = NO_FF(d, a, b, c, 0xE8C7B756, 12, orig);
  ...
  words[2] = NO_FF(c, d, a, b, 0x242070DB, 17, orig);
  ...
  words[3] = NO_FF(b, c, d, a, 0xC1BDCEEE, 22, orig);
  ...
  words[4] = NO_FF(a, b, c, d, 0xF57C0FAF, 7, orig);
  ...
  words[5] = NO_FF(d, a, b, c, 0x4787C62A, 12, orig);
  ...
  words[6] = NO_FF(c, d, a, b, 0xA8304613, 17, orig);
  ...
  words[7] = NO_FF(b, c, d, a, 0xFD469501, 22, orig);
  ...
  words[8] = NO_FF(a, b, c, d, 0x698098D8, 7, orig);
  ...
  words[9] = NO_FF(d, a, b, c, 0x8B44F7AF, 12, orig);
  ...
  words[10] = NO_FF(c, d, a, b, 0xFFFF5BB1, 17, orig);
  ...
  words[11] = NO_FF(b, c, d, a, 0x895CD7BE, 22, orig);
  ...
  words[12] = NO_FF(a, b, c, d, 0x6B901122, 7, orig);
  ...
  words[13] = NO_FF(d, a, b, c, 0xFD987193, 12, orig);
  ...
  words[14] = NO_FF(c, d, a, b, 0xA679438E, 17, orig);
  ...
  words[15] = NO_FF(b, c, d, a, 0x49B40821, 22, orig);
...
int validate_candidates_cuda(const uint32_t *state, uint32_t *candidates,
                             uint8_t *found, const size_t batch_size) {
  // Error code to check return values for CUDA callsd
  cudaError_t err = cudaSuccess;

  uint32_t *d_state = NULL;
  err = cudaMalloc((void **)&d_state, sizeof(uint32_t) * 4);
  ...
  err = cudaMalloc((void **)&d_candidates, sizeof(uint32_t) * batch_size * 16);
  ...
  uint8_t *d_found = NULL;
  err = cudaMalloc((void **)&d_found, sizeof(uint8_t) * batch_size);
  ...
  err =
      cudaMemcpy(d_state, state, sizeof(uint32_t) * 4, cudaMemcpyHostToDevice);
  ...
  err = cudaMemcpy(d_candidates, candidates, sizeof(uint32_t) * batch_size * 16,
                   cudaMemcpyHostToDevice);

  if (err != cudaSuccess) {
    fprintf(stderr,
            "Failed to copy vector candidates from host to device (error code "
            "%s)!\n",
            cudaGetErrorString(err));
    exit(EXIT_FAILURE);
  }
  ...
  int blocksPerGrid = batch_size / threadsPerBlock;
  validateCandidates<<<blocksPerGrid, threadsPerBlock>>>(d_state, d_candidates,
                                                         d_found, batch_size);
